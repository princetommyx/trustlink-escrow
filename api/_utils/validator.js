 ...
 * @returns {{ isValid: boolean, local: string, intl: string, error?: string }}
 */
export function validateGhanaPhone(phone) {
  if (!phone || typeof phone !== 'string' && typeof phone !== 'number') {
    return { isValid: false, local: '', intl: '', error: 'Phone number is required.' };
  }
  ...
  const validPrefixes = ['024', '025', '053', '054', '055', '059', '020', '050', '026', '027', '056', '057', '028'];
  ...
  return { isValid: true, local, intl };
}

/**
 * Validates escrow transaction amount.
 ...
 * @returns {{ isValid: boolean, amount: number, error?: string }}
 */
export function validateAmount(amount, min = 1.00, max = 50000.00) {
  ...
  if (num < min) {
    return { isValid: false, amount: num, error: `Amount must be at least GH₵ ${min.toFixed(2)}.` };
  }
  if (num > max) {
    return { isValid: false, amount: num, error: `Amount cannot exceed GH₵ ${max.toLocaleString('en-US')}.` };
  }
  // Check decimal places (max 2)
  const parts = num.toString().split('.');
  if (parts.length > 1 && parts[1].length > 2) {
    return { isValid: false, amount: num, error: 'Amount cannot have more than 2 decimal places.' };
  }
  return { isValid: true, amount: Math.round(num * 100) / 100 };
}
